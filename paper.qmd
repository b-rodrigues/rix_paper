...
manages Python interpreter versions directly, [renv]{.pkg} does not restore the
version of [R]{.proglang} itself—installing the correct version must be done
separately using tools like [rig]{.pkg} [@rlib2023]. More critically, neither
handles system-level dependencies. If [sf]{.pkg} requires [GDAL]{.pkg} version
3.0 but the system has version 2.4 installed, neither tool can resolve this
conflict. Users must manually install system libraries, and the process differs
...
- [rixpress]{.pkg} orchestrates polyglot analytical pipelines using [Nix]{.pkg}
  ...
  across R, Python, and Julia. [Python]{.proglang} users can use
  [ryxpress]{.pkg}, a direct port.
