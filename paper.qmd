...
help data scientists define per-project environments. This is common practice
for [Python]{.proglang} users, who have many tools at their disposal, such as
the built-in *virtual environments* module, but also sophisticated third-party
tools. One particularly popular tool is [uv]{.pkg}, which manages both
project-specific [Python]{.proglang} interpreters (enabling different Python
versions per project) and package libraries using a lock file mechanism.

In the [R]{.proglang} ecosystem, virtual environments are not a built-in
language feature as they are in [Python]{.proglang}, but tools like [renv]{.pkg}
provide equivalent functionality. [renv]{.pkg} captures the project's software
state and writes it to a lock file (called `renv.lock`), which includes the
exact versions of [R]{.proglang} and all required packages. This lockfile serves
as a blueprint for restoring the environment automatically, ensuring that others
can recreate the same setup with minimal effort. Below is an abbreviated example
of an `renv.lock` file:
...
However, both [renv]{.pkg} and [uv]{.pkg} have important limitations. While
[uv]{.pkg} manages Python interpreter versions directly, [renv]{.pkg} does not
restore the version of [R]{.proglang} itself—installing the correct R version
must be done separately using tools like [rig]{.pkg} [@rlib2023]. More
critically, neither [uv]{.pkg} nor [renv]{.pkg} handle system-level
dependencies. If [sf]{.pkg} requires [GDAL]{.pkg} version 3.0 but the system has
version 2.4 installed, neither tool can resolve this conflict. Users must
manually install system libraries, and the process differs across operating
systems. Further details are discussed in the [renv]{.pkg} documentation^[See
https://rstudio.github.io/renv/articles/renv.html.].

Pre-compiled binary packages (Python wheels or R binaries from Posit Package
Manager) can simplify installation by avoiding compilation, but they do not
eliminate the need for system libraries that would be required at runtime—these
must still be installed separately.

Other [R]{.proglang} packages provide similar functionality with different
trade-offs. [groundhog]{.pkg} [@simonsohn2023] and [rang]{.pkg} [@chan2023]
enable date-based package installation, retrieving packages as they existed on
CRAN at a specified date. The Posit Package Manager offers another approach
through dated CRAN snapshots. However, like [renv]{.pkg}, these tools focus on
[R]{.proglang} packages and do not address system-level dependencies or
[R]{.proglang} version management.

These tools represent significant progress in reproducibility within their
respective ecosystems, but they share a common limitation: none addresses
system-level dependencies or supports polyglot environments. A project using
[R]{.proglang}, [Python]{.proglang} [@python3], and system tools like
[Quarto]{.pkg} or \LaTeX requires coordinating multiple package managers
([renv]{.pkg} for R, [uv]{.pkg} for [Python]{.proglang}, manual installation for
system tools), each with its own configuration and potential for conflicts.
...
4. Single-language focus: While [Docker]{.pkg} can support polyglot
  ...
  standard way to specify that one step should run in a [Python]{.proglang}
...
5. Incomplete Reproducibility: Reproducibility should be viewed as a spectrum
   ...
   provides strong run-time reproducibility, achieving verifiable, deterministic
   artefacts requires pinning image digests, freezing dependencies, and using
   deterministic build systems such as [Nix]{.pkg} for full control over both
   temporal and spatial dimensions [@Malka_2024].
