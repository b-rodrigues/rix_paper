...
abstract: |
  ...
  together. We present two [R]{.proglang} packages that leverage [Nix]{.pkg} to
  ...
  orchestrate polyglot pipelines. Each computational step runs in its own
  ...
  while maintaining bit-for-bit reproducibility. Beyond, it supports
  collaboration across heterogeneous computational environments, and ensures
  that analyses can be reproduced years into the future.
...
package sets and dated snapshots. Should maintenance cease (which is unlikely,
as I directly use it myself), users could seamlessly transition to upstream
`nixpkgs` commits, though with trade-offs: less frequent [R]{.proglang} and
[Julia]{.proglang} package updates. The fork's value lies not in creating
dependency, but in reducing friction through testing infrastructure and curated
"known-good" dates. Thus, the fork represents an enhancement to user experience
rather than a critical single point of failure—the underlying [Nix]{.pkg}
infrastructure and official `nixpkgs` repository remain the foundation, ensuring
long-term viability of environments created with [rix]{.pkg}.
...
[R]{.proglang}, and dark blue nodes are Quarto derivations.](images/dag.png){#fig-dag}
