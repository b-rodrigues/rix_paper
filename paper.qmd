...
following vector, `r set.seed(1234);sample(seq(1, 10), 5);set.seed(NULL)`, in
versions prior to 3.6, the result will be different. This is a documented change
as shown in the R version 3.6 release
...
We recommend `path_subshell` to be a subfolder of the project. Using `rix()`
we generate a `default.nix` file that will provide a shell with R version 3.5.3
when built. We then use `rix_init()`, which generates an `.Rprofile` file in
the same subfolder as where the subshell's `default.nix` is located. This `.Rprofile`
file ensures that the R session that will be spawned from this subshell does not
get any interference from the main session it gets called from:
...
We can now use this shell to generate our vector of random numbers using `with_nix()`:
...
The `expr` argument of `with_nix()` can be arbitrarly complex, but must
be wrapped inside a `function()` call. The value of `out_subshell` is
`r out_subshell`, as is indeed different than the one generated from a
recent version of R.
