...
Under the hood, [rix]{.pkg} implements version pinning (such as `"AER@1.2-8"`) and GitHub package retrieval (such as `fusen`) by bypassing the default `nixpkgs` package set. For GitHub packages, [rix]{.pkg} generates Nix expressions that fetch the repository at the specified commit hash and build it within Nix's isolated sandbox, ensuring the package remains reproducible even if the repository's main branch changes. For version pinning, [rix]{.pkg} generates custom derivations that fetch the specific source tarball from the CRAN Archive (`https://cran.r-project.org/src/contrib/Archive/`) and compile it using the standard Nix builder function `pkgs.rPackages.buildRPackage`.

While manually overriding package versions via the `@` syntax is convenient, users must be aware of its limitations. First, the CRAN Archive only indexes packages that have been superseded by newer releases; requesting the current active version on CRAN using the `@` syntax will fail with an HTTP 404 error since it does not yet reside in the `/Archive/` directory. Second, Nix resolves all package dependencies (other [R]{.proglang} packages and system libraries) using the versions provided by the holistically pinned `nixpkgs` snapshot `date` requested in the `rix()` call, rather than the original release date of the overridden package. This can lead to build failures if the snapshot dependencies are incompatible. For example, installing `dplyr@1.2.0` on a snapshot from `2025-10-20` will fail if `dplyr` 1.2.0 requires `rlang` ($\ge$ 1.1.7) but the snapshot only provides `rlang` 1.1.6. Conversely, older packages may fail to compile if system libraries in the snapshot have introduced breaking changes. Consequently, manually pinning package versions using the `@` syntax should generally be discouraged. The recommended approach is to select a single, holistic `nixpkgs` snapshot `date` where all required package versions are naturally co-compatible and tested together.
...
For workflows requiring custom compiled binaries (such as a custom C++ analysis program), users have two main integration options:

1. **Interactive compilation**: By adding standard compiler toolchains (such as `gcc`, `clang`, `gmake`, or `cmake`) to the `system_pkgs` argument of `rix()`, the resulting Nix shell provides a complete, hermetic environment for compiling the custom software locally. The user or a workflow script can run compilation commands (for example, `g++ -O3 -o my_analysis analysis.cpp`) inside the shell, guaranteeing that the exact same compiler version and system libraries are used on every rebuild.
