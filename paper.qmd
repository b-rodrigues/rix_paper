...
To define a variable in the [Nix]{.proglang}, the `let` keyword is used, so
the first lines of this expression define a variable called `pkgs` and
`system_pkgs`. `pkgs` is defined as the set of packages to be used in
this derivation. In this example, the package expressions are going to be pulled
from the `nixpkgs`
