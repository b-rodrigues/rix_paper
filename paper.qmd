...
installation but do not eliminate the need for system libraries required at
...
It should be further noted that some packages may build successfully, but then
fail at runtime as they try to further install runtime dependencies on first
run. One such example is [torch]{.pkg}.] According to Repology, a service
...
reproducibility assurance that [Nix]{.pkg} currently lacks.
...
([boost]{.pkg}), a pull request was submitted to the official `nixpkgs`
repository. I backported the fix to the `rstats-on-nix` fork, making RStudio
available to users of [rix]{.pkg} earlier than upstream, as merging PRs in the
official repository can take some time.
...
complementing the public [Nix]{.pkg} cache. Instructions for using Cachix are in
...
effectively demonstrates the capabilities of [rixpress]{.pkg}. It should also
be noted that the code [Julia]{.proglang}, [Python]{.proglang} and [R]{.proglang}
