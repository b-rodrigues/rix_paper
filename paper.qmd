...
`functions.R`). Further details are detailed step-by-step instructions are provided
...
As a sidenote: [Python]{.proglang} users that wish to use [ryxpress]{.pkg} 
would define the pipeline exactly the same way, as an [R]{.proglang} script.
[ryxpress]{.pkg} calls [R]{.proglang} and [rixpress]{.pkg} under the
hood to generate the `pipeline.nix` file and then build it.
