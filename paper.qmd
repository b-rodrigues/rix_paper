...
pipeline, each step is defined as a Nix derivation, providing two key benefits:
  ...
  Python step can run in a pure Python environment, an [R]{.proglang} step in an
  [R]{.proglang} environment, and a Quarto rendering step in yet another, all
  within the same pipeline.
  2. Deep Reproducibility: Each step is a hermetically sealed Nix derivation
  whose output is cached in the Nix store based on the hash of all its inputs.
  All artifacts are direct children of the computational environment (because
  the computational environment is actually a dependency of the artifacts),
  ensuring they are rebuilt if the environment changes, keeping environment and
  outputs always in sync.

The user defines the pipeline in an [R]{.proglang} script using functions
inspired by [targets]{.pkg}. For example, a simple pipeline might read data with
[Python]{.proglang} (`polars`), filter it, process it with [R]{.proglang}
(`dplyr`), and compile a Quarto document. The pipeline is defined as a list,
typically in a file called `gen-pipeline.R` and its execution environment in a
file called `gen-env.R`. Templates of these files can be created using
`rxp_init()`. On a machine that has [Nix]{.pkg} installed it is possible to
bootstrap an environment using a temporary shell:
...
\end{CodeInput}
```
One can start the R interpreter, and then call `rxp_init()`:
...
The required execution environment, defined in `gen-env.R`, is as follows:
...
Using the same temporary shell, users can now generate the `default.nix` file
by calling `source("gen-env.R")`. Users can now quit the temporary shell, and
build the environment by calling `nix-shell` in a terminal. Once the
development (or execution) environment has been built, users can enter it,
and call `source("gen-pipeline.R")` to generate a file called `pipeline.nix`
which is the [Nix]{.pkg} expression that defines the entire reproducible
pipeline.

The environment also includes the [ggdag]{.pkg} package [@ggdag], so it's
possible to view a visual representation of the pipeline (yellow nodes are
[Python]{.pkg} and blue nodes are [R]{.pkg} derivations respectively):

![Graphical representation of the pipeline](dag.png)

Running the script above does not execute the pipeline; it generates a
`pipeline.nix` file, which declaratively defines the workflow.
...
[Nix]{.pkg} executes each step in order, building dependencies as needed.
Outputs are cached, so subsequent runs only recompute steps with changed inputs
or code. The build process can also be executed in parallel, if the the pipeline
has independent branches. This provides the efficiency of [targets]{.pkg} with
multi-language support and bit-for-bit reproducibility. Artifacts can be
inspected interactively in [R]{.proglang} using `rxp_read("artifact_name")` or
`rxp_load("artifact_name")`.
...
[rixpress]{.pkg} includes several additional features not covered here for the
sake of brevity.

It is also possible to configure popular IDEs to work interactively and
seamlessly with both [rix]{.pkg} and [rixpress]{.pkg}, enabling a smooth,
