...
language [@r-core-team]. More careful authors might also specify the version of
...
function provides a concise summary of the software environment, including the [R]{.proglang}
...
installing the correct package versions --- a process that becomes difficult
when packages depend on system-level libraries. For example, the [sf]{.pkg}
package requires [GDAL]{.pkg}, [GEOS]{.pkg}, and [PROJ]{.pkg} libraries.
...
define per-project environments. This is common for [Python]{.proglang}
[@python3] users, who have many tools at their disposal, such as the built-in
*virtual environments* module, or third-party tools like [uv]{.pkg} [@uv], which
manages both project-specific interpreters and package libraries using a lock
file mechanism.

In the [R]{.proglang} ecosystem, virtual environments are not built-in as they
are in [Python]{.proglang}, however tools like [renv]{.pkg} [@renv] provide equivalent
...
Other [R]{.proglang} packages such as [groundhog]{.pkg} [@simonsohn2023] and
[rang]{.pkg} [@chan2023] enable date-based package installation, while the Posit
Package Manager provides dated CRAN snapshots. However, like [renv]{.pkg}, these
focus on [R]{.proglang} packages and do not address system-level dependencies or [R]{.proglang} version
management. Furthermore, using pre-compiled binaries of packages simplifies
installation but does not eliminate the need for system libraries required at
runtime. These tools represent significant progress in reproducibility within
their ecosystems, and this is not a shortcoming of their design—their scope is
intentionally limited to language-specific dependency management. Nevertheless,
they share a common limitation for researchers who need full-stack
reproducibility: none handles system-level dependencies or the increasingly
polyglot nature of modern data science.
...
entire repository to a specific [Git]{.pkg} commit, guaranteeing that the
...
execution [@wurmus2018]. While this hybrid approach ensures that all
...
gaining full build-time reproducibility guarantees. As we will show,
...
Achieving this gold standard, however, required the masterful integration of
multiple complex and disparate tools: [renv]{.pkg} for managing [R]{.proglang}
packages, [Docker]{.pkg} for containerizing the system environment, and
[Make]{.pkg} for orchestrating the analysis. This demonstrates that while a high
degree of reproducibility is attainable, it requires a heroic effort, forcing
researchers to become experts in a brittle and complex toolchain.
...
[Nix]{.pkg} can provide, within a single framework, functionality that is
commonly distributed across separate tools such as [Docker]{.pkg} for
deployment, [renv]{.pkg} for language-specific package management, and
[Make]{.pkg} for workflow orchestration. However, it has a steep
learning curve, with its own functional language for declaratively defining
software builds and configurations. While this ensures reproducibility, it also
creates a barrier to adoption.

To make [Nix]{.pkg} accessible to researchers, we developed two [R]{.proglang}
packages:

- [rix]{.pkg} generates [Nix]{.pkg} expressions from intuitive [R]{.proglang}
  function calls, eliminating the need to learn the [Nix]{.proglang}
  language—a purely functional language that, as we discuss in Section
  @sec-nix, represents a significant barrier to adoption. [rix]{.pkg} handles
...
with traditional imperative toolchains. Section 6 concludes with discussion of
future directions.
...
At its core, [Nix]{.pkg} treats software builds as pure functions, a concept
borrowed from functional programming. A pure function always produces the same
output for a given input—for example, if $f(x) = x^2$, then $f(2) = 4$
invariably holds. Likewise, [Nix]{.pkg} guarantees that identical inputs (source
...
[Nix]{.pkg} installs all packages into a dedicated directory called the
...
However, long-term reproducibility depends not only on the continued accessibility of the
`nixpkgs` repository (or a suitable fork, as discussed in Section @sec-fork),
but also on the availability of source code for all packages in the dependency
tree. While `nixpkgs` documents the precise version information and
cryptographic hashes needed to verify source code integrity, the actual source
files must still be retrievable from their original locations (or mirrors) at
build time. If a source repository becomes unavailable, users would need to
locate the source code themselves and manually supply it to the build process.

This limitation is noteworthy because [Guix]{.pkg}, [Nix]{.pkg}'s sibling
functional package manager, has addressed this concern through integration with
Software Heritage [@courtes2024; @softwareheritage2025]. [Guix]{.pkg} can
automatically fall back to archived versions of source code when original
locations become unavailable, providing an additional layer of long-term
reproducibility assurance that [Nix]{.pkg} currently lacks. However,
[Guix]{.pkg} is only supported on Linux, making it unsuitable for researchers
working on macOS or Windows (where WSL2 support for [Guix]{.pkg} is non-trivial
to configure).
...
issues but these tools are not alternatives to one another. Thus, a powerful
...
Under the hood, [rix]{.pkg} implements version pinning (such as "[AER]{.pkg}@1.2-8") and GitHub package retrieval (such as [fusen]{.pkg}) by bypassing the default `nixpkgs` package set. For GitHub packages, [rix]{.pkg} generates [Nix]{.pkg} expressions that fetch the repository at the specified commit hash and build it within [Nix]{.pkg}'s isolated sandbox, ensuring the package remains reproducible even if the repository's main branch changes. For version pinning, [rix]{.pkg} generates custom derivations that fetch the specific source tarball from the CRAN Archive (`https://cran.r-project.org/src/contrib/Archive/`) and compile it using the standard [Nix]{.pkg} builder function `pkgs.rPackages.buildRPackage`.

While manually overriding package versions via the `@` syntax is convenient, users must be aware of its limitations. First, the CRAN Archive only indexes packages that have been superseded by newer releases; requesting the current active version on CRAN using the `@` syntax will fail with an HTTP 404 error since it does not yet reside in the `/Archive/` directory. Second, [Nix]{.pkg} resolves all package dependencies (other [R]{.proglang} packages and system libraries) using the versions provided by the holistically pinned `nixpkgs` snapshot `date` requested in the `rix()` call, rather than the original release date of the overridden package. This can lead to build failures if the snapshot dependencies are incompatible. For example, installing [dplyr]{.pkg}@1.2.0 on a snapshot from `2025-10-20` will fail if [dplyr]{.pkg} 1.2.0 requires [rlang]{.pkg} ($\ge$ 1.1.7) but the snapshot only provides [rlang]{.pkg} 1.1.6. Conversely, older packages may fail to compile if system libraries in the snapshot have introduced breaking changes. Consequently, manually pinning package versions using the `@` syntax should generally be discouraged. The recommended approach is to select a single, holistic `nixpkgs` snapshot `date` where all required package versions are naturally co-compatible and tested together.
...
trade-off being less frequent [R]{.proglang} package updates.
...
trigger the runs.] Nix’s deterministic dependency resolution guarantees
...
pipelines. By leveraging [Nix]{.pkg} as a unified build automation system,
[rixpress]{.pkg} enables polyglot workflows where each step runs in a
hermetically sealed environment. This ensures both spatial and temporal
...
While [rix]{.pkg} and [rixpress]{.pkg} (and [ryxpress]{.pkg}) represent
significant progress, current limitations remain. Debugging complex
[rixpress]{.pkg} pipelines is more challenging than troubleshooting
[targets]{.pkg} workflows, especially when failures occur deep in the
[Nix]{.pkg} build process. Visualization capabilities, though useful, currently
lack features such as automatic detection of outdated derivations.
Cross-platform reproducibility remains generally strong but is constrained on
macOS by proprietary framework dependencies. These challenges are active areas
for development and community contribution. Researchers adopting these tools
should do so with realistic expectations: they provide strong guarantees of
spatial and temporal reproducibility and scalable polyglot workflows, but they
require patience during the learning phase and may occasionally present
challenges that simpler tools avoid.
