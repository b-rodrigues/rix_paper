...
origins as a premier R-centric service and software supplier to a company that
...
### Containerization: Strengths and Limitations
...
orchestration is PiGx (Pipelines in Genomics), a collection of genomics
...
- **[rix]{.pkg}** provides an R-native (and Python-native via [ryxpress]{.pkg})
  declarative interface to [Nix]{.pkg}. It generates [Nix]{.pkg} expressions from
  intuitive function calls, eliminating the need to learn the [Nix]{.proglang}
  language. [rix]{.pkg} handles environment definitions including
  [R]{.proglang}, [Python]{.proglang}, [Julia]{.proglang}
  [@bezanson2017julia], system tools, and dependencies.

- **[rixpress]{.pkg}** orchestrates polyglot analytical pipelines using
  [Nix]{.pkg} as the build engine. Each step runs in its own hermetic environment
  with automatic caching and dependency tracking, enabling polyglot workflows
...
orchestration. Section 5 compares the declarative [Nix]{.pkg}-based approach
with traditional imperative toolchains. Section 6 discusses limitations, and
Section 7 concludes.
...
Under the hood, [rix]{.pkg} implements version pinning (such as "[AER]{.pkg}@1.2-8") and GitHub package retrieval by generating [Nix]{.pkg} expressions that fetch the repository at the specified commit hash and build it within [Nix]{.pkg}'s isolated sandbox. While the `@` syntax is convenient, it has limitations: the CRAN Archive only indexes superseded packages, and dependency resolution uses the holistic `nixpkgs` snapshot rather than the original release date of the overridden package, which can lead to build failures. The recommended approach is to select a `nixpkgs` snapshot date where all required package versions are naturally co-compatible.
...
at following
[link](https://github.com/b-rodrigues/rix_paper)^[\url{https://github.com/b-rodrigues/rix_paper}].
...
section^[\url{https://github.com/b-rodrigues/rix_paper/blob/master/repro-script/repro_script.sh}].
