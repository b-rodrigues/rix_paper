...
system-level reproducibility (Python interpreter, system libraries) with
[uv]{.pkg}'s comprehensive package availability. While this hybrid approach
sacrifices some of Nix's deterministic build guarantees for Python packages, it
provides a pragmatic solution when the needed packages are unavailable in
`nixpkgs`.
...
artifacts) it provides a pragmatic balance: comprehensive [Python]{.proglang}
...
setup instructions and a detailed walkthrough are provided in Appendix Section
@sec-polyglot-example.)
...
imported in this way is stored within the Nix store, ensuring reproducibility
but also persisting the data in the build environment. The function `rxp_qmd()`
is used to compile a [Quarto]{.pkg} document (there is also `rxp_rmd()` which
serves the same purpose for [R Markdown]{.pkg} documents).
