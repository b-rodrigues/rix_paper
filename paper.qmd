...
build container images entirely with its own toolchain (using
`dockerTools.buildImage`), bypassing the imperative `Dockerfile` approach
altogether and producing images with stronger reproducibility guarantees.
Alternatively, running a [Nix]{.pkg} environment inside a container based on
another Linux image can be a simpler way to integrate [Nix]{.pkg} with existing
CI tooling, avoiding the need to install derivations into the host system's
`/nix` directory.
...
pipelines that uses [Guix]{.pkg} for environment management and
[Snakemake]{.pkg} for workflow execution [@wurmus2018]. While this hybrid
approach ensures that all tools—including [Snakemake]{.pkg} itself—are
reproducibly installed via [Guix]{.pkg}, the workflow execution occurs
*outside* the package manager's build system. This means the analysis code runs
in the [Guix]{.pkg}-provided environment but does not benefit from the additional
reproducibility guarantees that come from executing within the sandboxed,
hermetic build process of a derivation. [BioNix]{.pkg} [@bedo_unifying_2020] takes this
further by modelling bioinformatics workflow stages as pure functions directly
within [Nix]{.pkg}, gaining the full reproducibility guarantees that the build
system provides. As we will show, [rixpress]{.pkg} follows a similar approach:
each pipeline step is itself a [Nix]{.pkg} derivation, meaning the entire
analysis—not just the environment—is subject to the same deterministic build
guarantees. Unlike [BioNix]{.pkg}, which requires users to write [Nix]{.proglang}
expressions, [rixpress]{.pkg} provides an accessible [R]{.proglang} (or
[Python]{.proglang}) interface for defining pipelines.
...
both interactive development and non-interactive, automated analysis. However,
long-term reproducibility depends not only on the continued accessibility of the
