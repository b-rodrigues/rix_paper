---
title: "A Framework for Reproducible Data Science: Managing Environments and Pipelines with Nix, rix, and rixpress"
format:
    jss-pdf:
        keep-tex: true
    jss-html: default
author:
  ...
  - name: Philipp Baumann
    affiliations:
      - Plus Affiliation
abstract: |
  To create a reproducible analysis, it is not enough to write clean, well-documented code. 
  One must also ensure that all software dependencies are clearly declared and, ideally, 
  provide a simple way to install them. Several tools exist for this purpose, such as the 
  containerization solution [Docker]{.proglang}. A complete solution, however, requires not only 
  managing the computational environment but also orchestrating the execution of the analysis itself
  in a robust and automated way. This paper presents a comprehensive framework based on the [Nix]{.pkg} 
  package manager to solve both problems. We first introduce the [R]{.proglang} package [rix]{.pkg}, 
  which simplifies the creation of reproducible environments with [Nix]{.pkg}. We then introduce 
  [rixpress]{.pkg}, a second [R]{.proglang} package that leverages these environments to define and 
  execute complex, multi-language analytical pipelines. Together, these tools provide a powerful, 
  unified approach to achieving high standards of computational reproducibility.
...
A final step towards the gold standard is to use a build
...
including the programming language that was used, as well as a build automation
tool. It should also be noted
...
to [R]{.proglang} programmers, we developed the [rix]{.pkg} and [rixpress]{.pkgs} packages which we
...
older version of the package and also installs several TeXLive packages for
authoring documents with \LaTeX, the development versions of [rix]{.pkg} and
[fusen]{.pkg} (downloaded from Github), and finally the Positron integrated
development environment. You can see the resulting `default.nix` file in the
@sec-appendix.
...
Every day, the [R]{.proglang} packages set gets updated, and the result commited
to a dated branch using scripts running on GitHub Actions (a continuous
integration and deployment platform). This has been going on since the 14th of
December 2024 (see
...
But this doesn’t mean that we recommend users use any date from the
...
if this succeeds is the date added through a PR to the list of available dates
on `{rix}`. This means that we also monitor the health of popular [R]{.proglang}
packages and can propose a fix upstream if a package stops building
successfully, which is something that can sometimes happen. For example,
recently RStudio was in a broken state due to an issue in one of its
dependencies, `boost`. This is not the fault of anyone in particular: it's just
that packages get updated and packages that depend on them should get updated as
well: but if that doesn't happen quickly enough, the `nixpkgs` maintainer faces
a conundrum. Either he or she doesn't update the package because it breaks
others, but not updating a package could be a security vulnerability, or he or
she updates the package, but now other, perhaps less critical packages are
broken and need to be fixed, either by their upstream developers, or by the
`nixpkgs` maintainer of said packages. In the case of RStudio a fix was proposed
and promptly merged, but if users wanted to install RStudio during the time it
took to fix it, they would have faced an error message.

However, thanks to our fork, we can back-port fixes and so if users try to
install RStudio using the date `"2025-02-10"` (which is when it was broken
on upstream `nixpkgs`), it'll work.

We spent quite some time backporting fixes: we went back all the way back to
March 2019. To do so, we start by checking out a `nixpkgs` commit on selected
dates, then we "update" the [R]{.proglang} packages set by using the Posit CRAN
and Bioconductor daily snapshots at that date. Then, we backport as many fixes
as possible, and ensure that a selection of popular packages work on both
x86-linux (which includes Windows, through WSL2) and aarch64-darwin (which is
how Apple Silicon Apple computers are designated as). Then we commit everything
to a dated branch of the `rstats-on-nix/nixpkgs` fork. Users can consult all the
available dates by running: `rix::available_dates()`.

An issue with forking `nixpkgs` though, is that by backporting fixes, we
essentially create new [Nix]{.pkg} packages that are not included in upstream,
and thus, these are not built by Hydra, Nix's CI platform which builds binary
packages. In practice this means that anyone using our fork will have to compile
many packages from source, which can be very time consuming. However, we also
provide a binary cache of packages sponsored by
[Cachix](https://www.cachix.org/) which complements the default, public
[Nix]{.pkg} cache. We provide instructions on how to use Cachix in the package's
Readme file. Using Cachix speeds up the installation process of packages
tremendously, as they simply get downloaded instead of being built (we build
a selection of packages only however, not the whole of CRAN and Bioconductor).

## Orchestrating the Workflow with rixpress {#sec-rixpress}

Defining a reproducible environment with [rix]{.pkg} solves the first major challenge of reproducibility. The second is to reliably and efficiently execute the analysis workflow within that environment. This is the role of the [rixpress]{.pkg} package.

As mentioned in the introduction, a build automation tool like [targets]{.pkg} is invaluable for managing complex analyses. It automatically tracks dependencies between code and data objects, caching results and only re-computing steps that have changed. One can run a [targets]{.pkg} pipeline inside a Nix environment to make it reproducible. However, this approach has limitations: the entire pipeline must run in a single environment, and orchestrating steps across different languages (e.g., [R]{.proglang} and [Python]{.proglang}) requires manual handling via packages like [reticulate]{.pkg}.

[rixpress]{.pkg} overcomes these limitations by using Nix not just as a package manager, but as the build automation engine itself. In a [rixpress]{.pkg} pipeline, each step is defined as a Nix derivation. This powerful abstraction provides two key benefits:

1.  True Polyglot Pipelines: Each step can be assigned its own, distinct Nix environment. A Python step can run in a pure Python environment, an [R]{.proglang} step in an [R]{.proglang} environment, and a Quarto rendering step in yet another, all within the same pipeline.

2.  Deep Reproducibility: Because each step is a Nix derivation, its execution is hermetically sealed and its output is cached in the Nix store based on the hash of all its inputs. This provides caching and reproducibility at a level deeper than most other tools. On top of this, all artifacts built by the pipeline are direct children of the computational environment: should the environment change in any way, all the artifacts would get rebuilt, ensuring that the environment and the artifacts are never out of sync.

The user defines the pipeline in a simple [R]{.proglang} script, using functions heavily inspired by [targets]{.pkg}. Let's consider a simple pipeline that reads data with Python (`polars`), filters it, passes it to [R]{.proglang} (`dplyr`) for further analysis, and then compiles a Quarto document.

First, the pipeline is defined in an [R]{.proglang} script as a list of `rxp_*` function calls:

```{r, eval=FALSE}
# In a file like 'pipeline.R'
library(rixpress)

# Define the sequence of steps
pipeline_steps <- list(
  # Step 1: Read data using Python in its own environment
  rxp_py_file(
    name = mtcars_pl,
    path = "data/mtcars.csv",
    read_function = "lambda x: polars.read_csv(x, separator='|')",
    nix_env = "python-env.nix" # A Nix file defining the Python env
  ),
  ...
  rxp_py(
    ...
    nix_env = "python-env.nix"
  ),

  # Step 3: Convert the Python pandas DataFrame to an R data.frame
  # This uses the main project environment ('default.nix')
  rxp_py2r(
    name = mtcars_r,
    expr = mtcars_filtered
  ),

  # Step 4: Process the data in R
  rxp_r(
    name = mtcars_summary,
...
rixpress(pipeline_steps, project_path = ".")
```

The user first creates the necessary environment files (e.g., `default.nix`, `python-env.nix`) using `rix`. 
Then, running the script above does not execute the pipeline but instead generates a `pipeline.nix` file, which is the complete, declarative definition of the workflow.

To build the pipeline, the user runs:

```{r, eval=FALSE}
rxp_make()
```

[Nix]{.pkg} then executes each step in the correct order, building any dependencies as needed. Because the output of each step is cached, subsequent runs of `rxp_make()` will only re-execute steps whose code or inputs have changed. This provides the same efficiency as [targets]{.pkg} but with the added benefits of multi-language support and the bit-for-bit reproducibility of Nix. Artifacts from any step can be loaded into an interactive [R]{.proglang} session for inspection using `rxp_read("artifact_name")` or `rxp_load("artifact_name")`.

For Python users, there is also a port of [rixpress]{.pkg}, called [ryxpress]{.pkg} that uses the same definition of the pipeline as an R script, but allows building the pipeline and viewing the outputs from a Python session.
...
Actions on each push, in a completely reproducible environment. For such an
...
in what we call a *subshell*. Suppose that you are running the latest version
...
On versions of [R]{.proglang} released after version 3.6, these two lines return
the following vector: `r out_r`, but in versions prior to 3.6, the result will
be different. This is a documented change as shown in the [R]{.proglang} version
3.6 release notes.^[https://stat.ethz.ch/pipermail/r-announce/2019/000641.html]
...
It is possible to use `with_nix()` from an [R]{.proglang} session that was
installed itself via Nix, or from a regular [R]{.proglang} installation.

## Conclusion

There are many tools to make one’s analyses reproducible, but [Nix]{.pkg}
distinguishes itself from these in that its main concern is in deploying the
complete set of packages that are *closed under the ''depends on'' relation*:
[Nix]{.pkg} does not simply install one piece of software, but its dependencies,
and their dependencies. This makes [Nix]{.pkg} extremely useful for reproducible
research, but because it solves a complex problem, it is a complex tool. With
[rix]{.pkg}, we wish to make using [Nix]{.pkg} more accessible to users of
[R]{.proglang} by providing a familiar interface and user experience. Because it
is possible to build reproducible development shells using [Nix]{.pkg}, it can
accomodate any use case: running scripts, pipelines, interactive [shiny]{.pkg}
applications, and serve [plumber]{.pkg} application programming interfaces.
