...
definition of the [Docker]{.prolgang} image.

It is possible to then execute the code of a study inside a so-called container,
which is an instance of an image. Executing, or running, these containers is
...
analysis scripts: [Docker]{.proglang} images are actually almost complete Linux
...
[Docker]{.proglang} image. The image can also be shared.
...
While [Docker]{.proglang} offers a great solution to the reproducibility
...
[Docker]{.proglang} will instead use the container to run a web server and the
web based application of a popular integrated development environment (IDE). So
for instance, it would be possible to launch the web interface of the RStudio
IDE, but not the operating system client of RStudio, which is usually what an R
programmer would use (or VS Code or Emacs...). A common approach seems to be to
...
[Docker]{.proglang}. Another challenge of using [Docker]{.proglang} is that,
as stated already, [Docker]{.proglang} images are actually almost complete
Linux operating systems, so experience with Linux is highly recommended if
one wishes to properly write a `Dockerfile`.

A final, and optional, step towards the gold standard is to use a build
automation tool such as [Make]{.proglang} to run the whole analysis when the
...
and even a build automation tool such as [Make]{.proglang}. To make
...
not the only reason [Nix]{.proglang} is very useful for reproducibility, but the
[Nix]{.proglang} package manager itself was designed with reproducibility in
mind. [Nix]{.proglang} is functional package manager, meaning that it uses
...
same inputs, regardless of where or when a package gets built.
We refer to @dolstra2004nix for a more technical discussion on [Nix]{.proglang}.
...
We recommend `path_subshell` to be a subfolder of the project. Using `rix()` we
...
as is indeed different than the one generated from a recent version of R.
